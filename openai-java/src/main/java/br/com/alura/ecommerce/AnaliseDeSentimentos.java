    ...
    private static String enviarRequisicao(Path arquivo) throws InterruptedException {
        ...
        while(tentativas != 2){
            try{
                ...
                switch (errorCode){
                    case 401 -> throw new RuntimeException("Erro com a chave da API!", e);

                    case 429 -> {
                        System.out.println("Rate Limit atingido! Nova tentaiva em instantes");
                        Thread.sleep(1000 * segundosParaProximaTentativa);
                        segundosParaProximaTentativa*= 2;
                    }

                    case 500, 503 -> {
                        System.out.println("API for da ar! Nova tentativa em instantes");
                        Thread.sleep(1000 * segundosParaProximaTentativa);
                        segundosParaProximaTentativa*= 2;
                    }
                }
        ...
        throw new RuntimeException("API Fora do ar! Tentativas realizadas sem sucesso!");
    }

    private static List<Path> carregarArquivosDeAvaliacoes() {
        try {
            var diretorioAvaliacoes = Path.of("openai-java/src/main/resources/avaliacoes");
            return Files
    ...
    private static void salvarArquivoDeAnaliseDeSentimento(Path arquivo, String analise) {
        try {
            var nomeProduto = arquivo
                    ...
                    .replace(".txt", "")
                    .replace("avaliacoes-", "");
            var path = Path.of("openai-java/src/main/resources/analises/analise-sentimentos-" +nomeProduto +".txt");
